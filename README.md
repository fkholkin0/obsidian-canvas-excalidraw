...
Canvas has no public event for its empty-area context menu, so this plugin monkey-patches `Canvas.prototype.showCreationMenu` directly. That's undocumented, unversioned internal API — it can change or break on any Obsidian update with no warning. If it stops working, the internals can be found again by grepping the strings inside Obsidian's own `obsidian.asar`.
